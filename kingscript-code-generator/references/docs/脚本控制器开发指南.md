...
> 序列化约束详见 `../../gotchas/README.md` 坑 7、8 及 `../../gotchas/serialization.md`，工具函数参考 `../templates/kwc-controller-template.md` 的 `toJavaSafe`。
...
const kwcController = new ResponseExample();
export { kwcController }
```
...
- 控制器类名是否与 `scriptFile` 一致？（如 `UserScriptController` ↔ `UserScriptController.ts`）
- 导出是否为 `export { kwcController }`？（不是 `export default`，也不是 `export { plugin }` 或 `export { ext }`）
- XML 中每个方法是否配置了 `permission`？（即使 `permitAll=true` 也需显式声明）
- URL 是否符合 `/{isv}/{app}/.../{resource}` 的 3 级以上规则？
- 通用运行时约束自检请对照 `../../gotchas/README.md` 坑位速查（KWC 专属坑 7-14，详见下方附录）

## 附录：运行时约束速查

以下约束的完整症状、原因和代码示例详见 `../../gotchas/README.md` 坑位速查及对应专题文档。

| 约束 | 坑位 | 专题文档 |
|------|------|----------|
| `QueryServiceHelper.query` 只有 4 参数签名，禁止第 5 参数 | 坑 13 | — |
| 分录字段必须带 `entryentity.` 前缀 | 坑 12 | `dynamicobject.md` |
| 字段名必须与实体元数据严格匹配 | 坑 6 | `dynamicobject.md` |
| 异常信息统一用 `'' + e`，禁止 `.message`/`.getMessage()` | 坑 4 | — |
| 顶层响应必须是对象，禁止直接返回数组 | 坑 7 | — |
| `response.ok` 入参中 JS 原生结构必须转为 Java 集合 | 坑 8、9 | `serialization.md` |
| 禁用 `?.`/`??` 等现代语法 | 坑 1 | — |
| 金额字段使用保守转换 | 坑 2 | `bigdecimal.md` |
| 禁止定义 `static` 成员 | 坑 3 | — |
| `DynamicObjectCollection` 禁止 `for-of` | 坑 5 | `dynamicobject.md` |
| `row.getDate()` 禁止使用 | 坑 14 | `dynamicobject.md` |
